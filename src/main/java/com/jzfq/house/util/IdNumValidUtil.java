...
public class IdNumValidUtil {
    ...
    */
    ...
    public static Map<String, Object> validIDMap(String idNum, boolean allowNull) throws ParseException {
        ...
        if(StringUtils.isBlank(idNum)){
            if(allowNull){
                ...
                resMap.put("code", 400);
                resMap.put("msg", "身份证号不能为空");
            }
            return resMap;
        }
        String idCard = idNum.toUpperCase();
        String validRes = IDCardValidate(idCard);
        if("该身份证有效！".equals(validRes)){
            resMap.put("code", 200);
            resMap.put("msg", validRes);
        }else{
            resMap.put("code", 400);
            resMap.put("msg", validRes);
        }
    ...
    public static boolean validID(String idNum, boolean allowNull, boolean isThrow) {
        if(StringUtils.isBlank(idNum)){
            if(allowNull){
                return true;
            }else{
        ...
        String idCard = idNum.toUpperCase();
        String validRes = null;
        try {
            validRes = IDCardValidate(idCard);
        } catch (ParseException e) {
            if(isThrow){
                e.printStackTrace();
            }else{
...
//        String IdCard="610821200612226111";
//        boolean res = validID(IdCard, true, false);
